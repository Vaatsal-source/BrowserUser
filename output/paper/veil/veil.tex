...
\definecolor{CiteGreen}{HTML}{16803D}
\definecolor{Ink}{HTML}{18334B}
\definecolor{Blue}{HTML}{2878B8}
\definecolor{Teal}{HTML}{168779}
\definecolor{Purple}{HTML}{8058B2}
\definecolor{Amber}{HTML}{B97611}
\definecolor{Rose}{HTML}{B44F73}
\usepackage[colorlinks=true,citecolor=CiteGreen,linkcolor=Ink,urlcolor=Blue]{hyperref}
\makeatletter
\renewcommand{\@biblabel}[1]{\textcolor{CiteGreen}{[#1]}}
\makeatother
\tikzset{flow/.style={rounded corners=3pt,draw=Blue,fill=Blue!9,line width=.7pt,align=center,font=\sffamily\small,text=Ink,minimum height=12mm,text width=4.5cm},
local/.style={flow,draw=Blue,fill=Blue!9},
privacy/.style={flow,draw=Purple,fill=Purple!10},
human/.style={flow,draw=Rose,fill=Rose!10},
cloud/.style={flow,draw=Amber,fill=Amber!13},
done/.style={flow,draw=Teal,fill=Teal!10},
arrow/.style={-{Latex[length=2mm]},line width=.9pt,draw=Ink},
edge/.style={font=\sffamily\scriptsize,text=Ink,fill=white,inner sep=2pt}}
\newcommand{\authorcell}[3]{\begin{minipage}[t][29mm][t]{.305\textwidth}\centering
{\normalfont\normalsize #1\par}\vspace{2pt}
{\normalfont\footnotesize\strut #2\par}
{\normalfont\footnotesize Delhi Technological University\par}
{\normalfont\footnotesize Delhi, India\par}\vspace{1pt}
...
\title{Veil: Towards Privacy-Preserving Browser Perception and Reasoning Across Dynamic Web Environments}
\author{\begin{tabular}{@{}ccc@{}}
\authorcell{Shreshth Rai}{Department of Applied Mathematics}{shreshthrai_mc24b06_039@dtu.ac.in}&
\authorcell{Aksh Aggarwal}{Department of Computer Science}{akshaggarwal_cse_25a01023@dtu.ac.in}&
\authorcell{Abhinash Chahataray}{Department of Electrical Engineering}{abhinashchahataray_ee_25a15011@dtu.ac.in}\\[2mm]
\authorcell{Himal Garg}{Department of Software Engineering}{himalgarg_se_25b01063@dtu.ac.in}&
\authorcell{Vaatsalya Srivastava}{}{vaatsalyasrivastava_ee_25a19036@dtu.ac.in}&
\authorcell{Shreya Kailash}{}{shreyakailash_25bd096@dtu.ac.in}
\end{tabular}}
\begin{document}
\maketitle
\begin{abstract}
...
Mind2Web contributes real-website demonstrations and evaluation across unseen websites and domains~\cite{mind2web}. Deng et al. describe a ``broad spectrum of user interaction patterns,'' which motivates varying Veil's form layouts and task families rather than evaluating a single fixed portal. WebVoyager studies multimodal interaction with live websites~\cite{webvoyager}; OSWorld broadens execution-based evaluation to computer tasks spanning applications~\cite{osworld}. These works motivate task diversity, but their published scores are not directly comparable with Veil's hypothetical privacy benchmark.
...
SeeClick identifies GUI grounding as ``the capacity to accurately locate screen elements based on instructions''~\cite{seeclick}. This distinction motivates measuring field localization separately from end-to-end completion. Set-of-Mark prompting associates visible regions with explicit marks~\cite{som}; a related design option for Veil is to attach public element identifiers to sanitized regions. Such identifiers aid grounding but do not themselves remove sensitive pixels or authorize execution.
...
MobileNetV2 provides a convolutional alternative based on inverted residuals and linear bottlenecks~\cite{mobilenet}. Comparing a suitable MobileNetV2-based region head with a MobileViT-based head would help isolate the accuracy--latency trade-off. Neither backbone should be selected solely from published classification results: the exported browser model must be assessed on Veil's own region labels.
...
PP-OCR emphasizes practical lightweight optical character recognition~\cite{ppocr}, while differentiable binarization addresses scene-text localization~\cite{dbnet}. TrOCR uses pretrained image and text transformers for recognition~\cite{trocr}. Together, these works motivate separating text-region detection, transcription, and sensitive-entity decisions. None of these OCR functions alone determines whether a recognized value is private. Veil would benchmark suitable local exports and independently test their privacy recall.
...
AgentDojo provides a dynamic setting for evaluating prompt-injection attacks and defenses in agent workflows~\cite{agentdojo}. Veil's proposed adversarial tests should similarly report both legitimate task completion and attacker-induced deviations. ReAct studies interaction between language-model reasoning and actions~\cite{react}; Veil adopts an observation--proposal--action loop while placing permission checks and private-value resolution in local code. These are design motivations, not evidence that either cited framework establishes Veil's privacy guarantees.
...
The executor applies a local predicate $\operatorname{Allow}(a_t,\tau_t,v_t,\pi)$ before resolving arguments. If the page changed, the origin is unauthorized, or a reference was revoked, the proposal is rejected and the page is re-observed. A successful local action yields sanitized verification evidence for the next step. A missing-information response pauses the same task rather than inventing a value or restarting with broader access. Figure~\ref{fig:userflow} presents this user journey.

\begin{figure*}[t]
\centering
\begin{tikzpicture}[node distance=7mm and 7mm]
\node[local] (obs) {\textbf{01 / OBSERVE}\\DOM, ARIA, screenshot\\Tab + geometry + version};
\node[privacy,right=of obs] (per) {\textbf{02 / PROTECT}\\Rules + CV + OCR + faces\\Validated local cache};
\node[human,right=of per] (rev) {\textbf{03 / REVIEW}\\Masked pixels + filtered text\\Approve exact payload};
\node[done,below=14mm of obs] (vault) {\textbf{06 / EXECUTE LOCALLY}\\Encrypted vault + references\\Fill authorized website fields};
\node[local,below=14mm of per] (check) {\textbf{05 / VALIDATE}\\Origin, target, version, scope\\Separate submission approval};
\node[cloud,below=14mm of rev] (model) {\textbf{04 / REASON REMOTELY}\\Hosted LLM / VLM\\Sanitized context only};
\draw[arrow] (obs)--(per);\draw[arrow] (per)--(rev);
\draw[arrow] (rev)--node[edge,right]{approved request}(model);
\draw[arrow] (model)--(check);\draw[arrow] (check)--(vault);
\draw[arrow] (vault)--node[edge,left]{verify; repeat}(obs);
\end{tikzpicture}
\caption{Veil system architecture. Blue: observation and control; purple: privacy; rose: human review; amber: remote reasoning; teal: private execution. Original values and visual features remain local to the reasoning pipeline; authorized filled values reach the destination website.}
...
\node[local] (start) {\textbf{1 / DEFINE THE TASK}\\Enter goal + website\\Select reviewed profile facts};
\node[privacy,right=of start] (browse) {\textbf{2 / OBSERVE AND MASK}\\Veil opens the website\\Private text and pixels filtered};
\node[human,right=of browse] (review) {\textbf{3 / REVIEW SHARING}\\Inspect exact outgoing context\\Approve, add masks, or deny};
\node[done,below=16mm of start] (finish) {\textbf{6 / REVIEW THE RESULT}\\Verify the completed form\\Approve submission separately};
\node[human,below=16mm of browse] (docs) {\textbf{5 / SUPPLY MISSING FACTS}\\Upload locally; confirm extraction\\Select new facts and resume};
\node[local,below=16mm of review] (act) {\textbf{4 / PLAN AND FILL}\\Remote plan; local reference input\\Check for missing information};
\draw[arrow] (start)--(browse);\draw[arrow] (browse)--(review);\draw[arrow] (review)--(act);
\draw[arrow] (act)--node[edge,above,yshift=8mm]{missing facts}(docs);
\draw[arrow] (docs)--node[edge,above,yshift=8mm]{resume; complete}(finish);
\draw[arrow] (act.south)--++(0,-.65)-|node[edge,pos=.25]{all facts available; complete}(finish.south);
\draw[arrow,dashed,draw=Rose] (docs.north)--node[edge,right]{fresh observation}(browse.south);
\end{tikzpicture}
\caption{User flow from task entry to final review. Missing-information requests preserve the task and require reviewed, explicitly selected facts before resumption. Denied sharing pauses the task; final submission has its own authorization.}
\label{fig:userflow}
\end{figure*}
...
\begin{figure}[t]
\centering
\begin{tikzpicture}[node distance=3mm,box/.style={draw=Teal,fill=Teal!9,rounded corners=2pt,align=center,text=Ink,text width=7.1cm,font=\sffamily\small,minimum height=8mm},arr/.style={-{Latex},draw=Teal,thick}]
...
\node[box,draw=Purple,fill=Purple!10,below=of b] (c) {Seeded layouts, languages, motion, and disclosure cases};
\node[box,below=of c] (d) {DOM spans + pixel masks + task outcomes + Q\&A};
\node[box,draw=Amber,fill=Amber!12,below=of d] (e) {Independent review and versioned evaluation manifest};
...
Documents are parsed or OCR-processed locally with page, size, and processing limits. Extracted candidates remain unavailable until the user reviews them. Document-specific totals retain their period and source and do not overwrite reusable profile facts.

LayoutLMv3 combines document text and image learning with a word--patch alignment objective~\cite{layoutlm}. It motivates an optional layout-aware extraction stage when a value's meaning depends on its position in a document. This would run within the local document-processing boundary and require a separate resource assessment; a general document model is not assumed to fit inside every browser worker. A narrow local calculation tool can combine authorized numeric references using decimal arithmetic, register its result as another reviewed or policy-permitted reference, and return only sanitized status.
...
\node[local] (capture) {\textbf{CAPTURE + BIND}\\DOM, pixels, tab/frame\\Geometry and page version};
\node[privacy,right=of capture] (detect) {\textbf{DETECT + REUSE}\\Invalidate changed regions\\Rules, OCR, CV, faces};
\node[privacy,right=of detect] (mask) {\textbf{SANITIZE + FREEZE}\\Merge masks; replace pixels\\Filter text, URLs, history};
\node[human,below=11mm of mask] (approve) {\textbf{EXACT-PAYLOAD REVIEW}\\Bind hash, provider, versions\\Edits require new approval};
\node[cloud,left=of approve] (reason) {\textbf{REMOTE INFERENCE}\\Sanitized labels + image\\Structured action or question};
\node[local,left=of reason] (validate) {\textbf{ACTION VALIDATION}\\Reference scope + target + origin\\Reject stale or invalid proposals};
\node[done,below=11mm of validate] (execute) {\textbf{LOCAL RESOLUTION}\\Lookup authorized private value\\Execute permitted action};
\node[local,right=of execute] (verify) {\textbf{FRESH VERIFICATION}\\Observe resulting website state\\Continue, pause, or finish};
\node[human,right=of verify] (pause) {\textbf{MISSING INFORMATION}\\Local document extraction\\Review facts; refresh references};
\draw[arrow] (capture)--(detect);\draw[arrow] (detect)--(mask);\draw[arrow] (mask)--(approve);
\draw[arrow] (approve)--(reason);\draw[arrow] (reason)--(validate);\draw[arrow] (validate)--(execute);\draw[arrow] (execute)--(verify);
\draw[arrow] (verify)--node[edge,above,yshift=8mm]{ask user}(pause);
\draw[arrow,dashed,draw=Teal] (verify.south)--++(0,-.6)-|node[edge,pos=.25]{next observation}([xshift=-5mm]capture.west)--(capture.west);
\end{tikzpicture}
\caption{Technical flow with explicit privacy and execution boundaries. Missing facts or consequential actions pause execution; invalid observations and proposals return to capture. Local cache reuse is permitted only while content, geometry, and policy bindings remain valid.}
\label{fig:technicalflow}
\end{figure*}

\subsection{Training Strategy and Optimization}
The proposed training program separates perception adaptation from policy enforcement. First, evaluate pretrained components on validation pages and establish rule-based baselines. Second, if UI-region recognition is inadequate, train a small detection head using region annotations. Third, calibrate detector thresholds on validation data, then freeze thresholds before held-out testing.

Guo et al. distinguish model confidence from calibrated correctness likelihood and evaluate post-processing methods including temperature scaling~\cite{calibration}. Veil would assess calibration independently for each detector and privacy category. A calibrated classifier score still does not certify that all sensitive content was detected; mandatory credential rules and conservative handling of unknown regions remain necessary. The hosted reasoning model can remain frozen; end-to-end training through the redaction boundary is not required.
...
Five controlled configurations are proposed. B0 sends raw screenshots and page text and serves only as a synthetic-data utility ceiling. B1 uses DOM and pattern redaction. B2 additionally runs OCR and face detection over each observation without change-aware reuse. B3 masks the entire image while retaining sanitized text. Veil-A combines multimodal redaction with validated caching. Veil-R adds user review to Veil-A and is evaluated separately.
...
Table~\ref{tab:main} presents one hypothetical privacy--utility trade-off. Veil-A is assigned 97.1\% micro F1, 99.2\% pixel coverage, and 91.0\% task success. The raw-input condition has higher hypothetical task success but no redaction coverage. Full-image masking covers every sensitive pixel while also masking every non-sensitive pixel; its reduced hypothetical task success illustrates why privacy coverage alone is insufficient.
...
Veil-A: Multimodal + cache & 97.1 & 99.2 & 7.4 & 91.0 & 110 & 240\\
Veil-R: Veil-A + review & -- & 99.8 & 8.6 & 92.0 & -- & --\\
\bottomrule
\end{tabular}
\end{table*}

The hypothetical B2 and Veil-A detection values are deliberately equal: cache reuse is intended to preserve outputs when valid while reducing redundant computation. Equality here is a design hypothesis, not a finding. Veil-R has no automatic F1 entry because manual additions would confound attribution. Its latency is omitted from that table because review introduces a separately measured human-time distribution.
...
\begin{table}[t]
\centering\small
\caption{Invented resource and interaction examples for Veil-A. \illustrative\ CPU/GPU and cold-start values would require device-specific profiling.}
\label{tab:resource}
\begin{tabular}{lr}
\toprule
...
\bibitem{mind2web} X. Deng et al., ``Mind2Web: Towards a Generalist Agent for the Web,'' arXiv:2306.06070, 2023. [Online]. Available: \url{https://arxiv.org/abs/2306.06070}
\bibitem{webvoyager} H. He et al., ``WebVoyager: Building an End-to-End Web Agent with Large Multimodal Models,'' arXiv:2401.13919, 2024. [Online]. Available: \url{https://arxiv.org/abs/2401.13919}
\bibitem{osworld} T. Xie et al., ``OSWorld: Benchmarking Multimodal Agents for Open-Ended Tasks in Real Computer Environments,'' arXiv:2404.07972, 2024. [Online]. Available: \url{https://arxiv.org/abs/2404.07972}
\bibitem{seeclick} K. Cheng et al., ``SeeClick: Harnessing GUI Grounding for Advanced Visual GUI Agents,'' arXiv:2401.10935, 2024. [Online]. Available: \url{https://arxiv.org/abs/2401.10935}
\bibitem{som} J. Yang et al., ``Set-of-Mark Prompting Unleashes Extraordinary Visual Grounding in GPT-4V,'' arXiv:2310.11441, 2023. [Online]. Available: \url{https://arxiv.org/abs/2310.11441}
...
\bibitem{mobilenet} M. Sandler et al., ``MobileNetV2: Inverted Residuals and Linear Bottlenecks,'' arXiv:1801.04381, 2018. [Online]. Available: \url{https://arxiv.org/abs/1801.04381}
...
\bibitem{ppocr} Y. Du et al., ``PP-OCR: A Practical Ultra Lightweight OCR System,'' arXiv:2009.09941, 2020. [Online]. Available: \url{https://arxiv.org/abs/2009.09941}
\bibitem{dbnet} M. Liao et al., ``Real-time Scene Text Detection with Differentiable Binarization,'' arXiv:1911.08947, 2019. [Online]. Available: \url{https://arxiv.org/abs/1911.08947}
\bibitem{trocr} M. Li et al., ``TrOCR: Transformer-based Optical Character Recognition with Pre-trained Models,'' arXiv:2109.10282, 2021. [Online]. Available: \url{https://arxiv.org/abs/2109.10282}
...
\bibitem{agentdojo} E. Debenedetti et al., ``AgentDojo: A Dynamic Environment to Evaluate Prompt Injection Attacks and Defenses for LLM Agents,'' arXiv:2406.13352, 2024. [Online]. Available: \url{https://arxiv.org/abs/2406.13352}
\bibitem{react} S. Yao et al., ``ReAct: Synergizing Reasoning and Acting in Language Models,'' arXiv:2210.03629, 2022. [Online]. Available: \url{https://arxiv.org/abs/2210.03629}
\bibitem{layoutlm} Y. Huang et al., ``LayoutLMv3: Pre-training for Document AI with Unified Text and Image Masking,'' arXiv:2204.08387, 2022. [Online]. Available: \url{https://arxiv.org/abs/2204.08387}
\bibitem{calibration} C. Guo, G. Pleiss, Y. Sun, and K. Q. Weinberger, ``On Calibration of Modern Neural Networks,'' arXiv:1706.04599, 2017. [Online]. Available: \url{https://arxiv.org/abs/1706.04599}
\bibitem{adamw} I. Loshchilov and F. Hutter, ``Decoupled Weight Decay Regularization,'' arXiv:1711.05101, 2017. [Online]. Available: \url{https://arxiv.org/abs/1711.05101}
\end{thebibliography}
\end{document}
